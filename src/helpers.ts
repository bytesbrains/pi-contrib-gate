...
export function hasUnpushed(cwd: string): boolean {
	...
	const r = exec(
		`git log gitea/${branch}..HEAD --oneline 2>/dev/null || git log origin/${branch}..HEAD --oneline 2>/dev/null || echo ""`,
		cwd,
	);
			...
			if (match) {
				...
				try {
					...
					const res = await fetch(
						`${apiUrl}/pulls?head=${encodeURIComponent(br)}&state=all&limit=1`,
						{ headers },
					);
...
export async function createPR(
	...
	if (url.includes("gitea") || url.includes("127.0.0.1:3001")) {
		const match = url.match(/[/:]([^/]+)\/([^/]+?)(?:\.git)?$/);
		if (!match)
			return { ok: false, error: `Cannot parse Gitea repo from: ${url}` };
