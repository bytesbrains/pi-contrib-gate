...
export const proposeTool = {
	...
	description:
		"Stage files, validate conventional commit, run quality checks, and commit.",
	parameters: Type.Object({
		message: Type.String({
			description: "Conventional commit message: type(scope): subject",
		}),
		body: Type.Optional(
			Type.String({ description: "Extended commit body with details" }),
		),
	}),
	async execute(
		_toolCallId: string,
		...
		if (!add.ok)
			return {
				content: [
					{ type: "text", text: `Failed to stage files: ${add.stderr}` },
				],
				isError: true,
				details: {},
			};
		...
		if (
			bestPractices.shortFrequentCommits &&
			bestPractices.guidanceText.length > 0
		) {
