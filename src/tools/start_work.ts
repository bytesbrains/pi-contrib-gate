...
export const startWorkTool = {
	...
	label: "Start Work",
	description:
		"Link work to a Gitea issue. Creates a feature branch if not already on one; otherwise links the issue to the current branch (e.g., for rework on an existing PR).",
	parameters: Type.Object({
		issue_id: Type.String({
			description: "Issue number or ID to link this work to",
		}),
		type: Type.Optional(
			Type.String({
				description: "Branch type: feat, fix, or chore (default: feat)",
			}),
		),
	}),
	async execute(
		...
		if (!["feat", "fix", "chore"].includes(branchType)) {
			return {
				content: [
					{
						type: "text",
						text: `Invalid branch type: "${branchType}". Use: feat, fix, or chore.`,
					},
				],
				isError: true,
				details: {},
		...
		if (config.requireIssueValidation) {
			...
			const issueLabels = Array.isArray(issue.labels)
				? (issue.labels as any[]).map((l: any) => l.name).join(", ")
				: "";
			...
			if (issueAssignee && issueAssignee !== "factory") {
				warnings.push(
					`⚠️  Issue #${issueId} is assigned to "${issueAssignee}". Verify you should be working on this.`,
				);
			}
			...
			if (isFeatureBranch) {
				...
				const alreadyLinked =
					getLinkedIssueId(ctx.cwd) === issueId &&
					currentBr.includes(`issue-${issueId}`);
			...
			if (!isClean(ctx.cwd)) {
				return {
					content: [
						{
							type: "text",
							text: "⚠️ Working tree is not clean. Commit or stash changes before starting new work.",
						},
					],
					isError: true,
					details: { clean: false },
			...
			if (pullRemote) {
				exec(
					`git pull --ff-only ${shellEscape(pullRemote)} dev 2>/dev/null || true`,
					ctx.cwd,
				);
			} else {
				exec(
					"git pull --ff-only gitea dev 2>/dev/null || git pull --ff-only origin dev 2>/dev/null || true",
					ctx.cwd,
				);
			}
			...
			if (!r2.ok) {
				return {
					content: [
						{ type: "text", text: `Failed to create branch: ${r2.stderr}` },
					],
					isError: true,
					details: {},
				};
		...
		if (pullRemote2) {
			exec(
				`git pull --ff-only ${shellEscape(pullRemote2)} dev 2>/dev/null || true`,
				ctx.cwd,
			);
		} else {
			exec(
