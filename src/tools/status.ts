...
export const statusTool = {
	...
	description:
		"Show current branch, commit status, uncommitted changes, and PR status.",
	...
	async execute(
		...
		const changes =
			status.ok && status.stdout ? status.stdout.split("\n").length : 0;
		...
		if (branch && !remote.exists) {
			...
			lines.push(
				`      → Run contrib_start_work(issue_id) to start fresh work.`,
			);
