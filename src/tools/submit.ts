...
export const submitTool = {
	...
	parameters: Type.Object({
		...
		base: Type.Optional(
			Type.String({ description: "Target branch (default: dev)" }),
		),
		remote: Type.Optional(
			Type.String({ description: "Git remote name (auto-detects)" }),
		),
	}),
	async execute(
		_toolCallId: string,
		...
		if (
			!hasUnpushed(ctx.cwd, config) &&
			!(globalThis as any).__contrib_lastHash
		) {
			return {
				content: [
					{
						type: "text",
						text: "No commits to push. Run contrib_propose() first.",
					},
				],
				isError: true,
				details: {},
		...
		const push = exec(
			`git push -u ${shellEscape(remoteName)} ${shellEscape(branch)}`,
			ctx.cwd,
		);
		if (!push.ok)
			return {
		...
		if (!pr.ok)
			return {
				content: [
					{
						type: "text",
						text: `Push succeeded but PR creation failed: ${pr.error}`,
					},
				],
				isError: true,
				details: { branch },
